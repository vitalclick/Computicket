...
interface Props {
  /** Pre-filled query in the search input. */
  initialQuery?: string;
  /** Pre-selected facet tab. */
  initialFacet?: string;
  /** Placeholder for the search input. */
  searchPlaceholder?: string;
  /** Accessible label for the search input. */
  searchAriaLabel?: string;
  /** Singular noun used in the Compass Answers copy ("event" by default,
   * pass "concert" for /concerts, etc). */
  entityLabel?: string;
}
...
function buildCompassAnswer({
  ...
  const plural = `${entityLabel}${total === 1 ? '' : 's'}`;
  if (query.trim().length === 0) {
    return `Showing ${total} live ${plural}. Try "Burna Boy Lagos", "Comedy this weekend", or filter by city to narrow down.`;
  }
  return `I found ${total} match${
    total === 1 ? '' : 'es'
          ...
          >
            <div className="row gap-3" style={{ alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  {buildCompassAnswer({ query, total: designed.length, entityLabel })}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="between mb-4">
              <div>
                <div className="eyebrow mb-2">
                  {query.trim()
                    ? `Also matching "${query.trim().toUpperCase()}"`
                    : 'Also worth a look'}
                </div>
